...
function addToArray(newImage) {
	if (count < 10) {
		...
		if (count >= 10) {
			...
			for (let i = 0; i < 10; i ++) {
				let imgId1a = "RadarImg" + i;
				document.getElementById(imgId1a).src = imageArray[i].src;
			}

			let j = 0;
			let timer = setInterval(function() {
				let imgId1b = "RadarImg" + j;
				document.getElementById(imgId1b).style.display = 'inline';
				j++;

				if (j == 10) {
					for (let k = 1; k < 10; k ++) {
						let imgId1c = "RadarImg" + k;
						document.getElementById(imgId1c).style.display = 'none';
					}
					j = 0;
				}
			}, 150);
